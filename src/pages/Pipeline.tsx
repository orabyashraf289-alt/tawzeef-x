...
import { Star, GripVertical, Users, Search, Filter, TrendingUp, Clock, Bot, Kanban, List, CheckCircle2, ArrowRight, ArrowUpDown, BarChart3, ClipboardCopy, ExternalLink, FileCheck, Mail, Loader2, Eye, Check, AlertTriangle, Settings, Sliders, ClipboardCheck } from "lucide-react";
...
function DroppableColumn({ 
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "flex flex-col min-w-[245px] w-[245px] lg:w-auto lg:flex-1 rounded-xl border border-border/50 bg-muted/20 transition-all duration-200",
    ...
    >
      <div className="p-3 border-b border-border/50">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5 min-w-0 flex-1">
            ...
            <span className="text-xs font-semibold text-foreground truncate" title={label}>{label}</span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            {onConfigure && (
              <Button
                variant="ghost"
                size="icon"
                className="w-5 h-5 rounded hover:bg-muted text-muted-foreground/60 hover:text-foreground transition-colors"
                onClick={(e) => {
                  e.stopPropagation();
                  onConfigure();
                }}
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
            <span className="flex items-center gap-0.5" title="معدل التحويل من إجمالي المتقدمين">
              <TrendingUp className="w-3 h-3 text-muted-foreground/60" />{conversionRate}%
            </span>
            {avgDays > 0 && (
              <span className="flex items-center gap-0.5" title="متوسط البقاء في هذه المرحلة">
                <Clock className="w-3 h-3 text-muted-foreground/60" />{avgDays}{tLabel.avgDays.charAt(0) === "A" ? "d" : "ي"}
              </span>
            )}
            {slaHours > 0 && (
              <span className="flex items-center gap-0.5 text-amber-600 dark:text-amber-400 font-semibold" title={`الحد الأقصى للمرحلة (SLA): ${slaHours} ساعة`}>
                <Clock className="w-3 h-3 text-amber-500 shrink-0" />{slaHours}س
              </span>
            )}
          </div>

          {/* Rules locks indicators with Tooltip */}
          <TooltipProvider>
            {(transitionRules.require_interview || transitionRules.require_ai_evaluation || transitionRules.require_assessment) && (
              <div className="flex items-center justify-between text-[9px] text-muted-foreground bg-muted/40 px-2 py-0.5 rounded border border-border/30 mt-1 select-none">
                <span className="font-medium shrink-0">الشروط:</span>
                <div className="flex items-center gap-1.5">
                  {transitionRules.require_interview && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Video className="w-3 h-3 text-blue-500 hover:scale-110 transition-transform cursor-help" />
                      </TooltipTrigger>
                      <TooltipContent className="text-xs">تطلب مقابلة مكتملة وتقييمها</TooltipContent>
                    </Tooltip>
                  )}
                  {transitionRules.require_ai_evaluation && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Bot className="w-3 h-3 text-purple-500 hover:scale-110 transition-transform cursor-help" />
                      </TooltipTrigger>
                      <TooltipContent className="text-xs">{`تطلب تقييم ذكي (الحد الأدنى: ${transitionRules.min_ai_score || 0}%)`}</TooltipContent>
                    </Tooltip>
                  )}
                  {transitionRules.require_assessment && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <ClipboardCheck className="w-3 h-3 text-emerald-500 hover:scale-110 transition-transform cursor-help" />
                      </TooltipTrigger>
                      <TooltipContent className="text-xs">تطلب إكمال الاختبار المرتبط</TooltipContent>
                    </Tooltip>
                  )}
                </div>
              </div>
      ...
      <div className="p-2 space-y-2 flex-1 overflow-y-auto max-h-[calc(100vh-320px)] min-h-[80px]">
        {children}
      </div>
    </div>
  );
...
export default function Pipeline() {
  ...
  const STAGES = useMemo(() => {
    let baseStages = activeStages.length > 0
      ...
      : FALLBACK_STAGES.map(s => ({ ...s, sla_hours: 0, transition_rules: {} }));

    if (statusFilter === "rejected") {
      baseStages = [...baseStages, { id: "مرفوض", label: "مرفوض", color: "#ef4444", sla_hours: 0, transition_rules: {} }];
    } else if (statusFilter === "deferred") {
      baseStages = [...baseStages, { id: "مؤجل", label: "مؤجل", color: "#f59e0b", sla_hours: 0, transition_rules: {} }];
    }
    return baseStages;
      ...
      <div className="p-4 lg:p-8 space-y-4">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/settings?tab=pipeline" className="shrink-0">
              <Button
                variant="outline"
                size="sm"
              ...
              >
                ...
                {dir === "rtl" ? "إعدادات المراحل" : "Stage Settings"}
              </Button>
            </Link>
            ...
            <div className="flex gap-3 overflow-x-auto pb-4 -mx-4 px-4 lg:mx-0 lg:px-0">
              {STAGES.map((stage) => (
                <DroppableColumn
                  ...
                  transitionRules={stage.transition_rules}
                  onConfigure={stage.id !== "مرفوض" && stage.id !== "مؤجل" ? () => setEditingStage(stage) : undefined}
                >
