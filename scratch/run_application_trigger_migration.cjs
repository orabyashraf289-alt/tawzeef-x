...
try {
  const fs = require('fs');
  fs.writeFileSync('scratch/temp_trigger.sql', sql);
  const result = execSync('npx supabase db query --linked --file scratch/temp_trigger.sql', { encoding: 'utf-8' });
  console.log("Migration executed successfully!");
  console.log(result);
  fs.unlinkSync('scratch/temp_trigger.sql');
} catch (e) {
