    ...
    <>
      ...
      >
        ...
        <motion.div variants={item}>
          <div className="relative rounded-3xl p-6 sm:p-7 border border-border/70 bg-card/70 backdrop-blur-xl shadow-lg overflow-hidden">
            {/* Ambient Background Aura */}
            <div className="absolute top-0 right-0 w-80 h-80 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none -z-10" />
            <div className="absolute bottom-0 left-0 w-60 h-60 bg-teal-500/10 rounded-full blur-2xl pointer-events-none -z-10" />

            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
              {/* Left: Greeting & Status Info */}
              <div className="space-y-2">
                <div className="flex items-center gap-2.5 flex-wrap">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 shadow-2xs">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500" />
                    </span>
                    <span>{locale === "en" ? "Live HR Intelligence" : "مباشر • مركز العمليات التوظيفية"}</span>
                  </span>

                  <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-muted/60 text-muted-foreground border border-border/40">
                    {locale === "en" ? "Encrypted Enterprise Cloud" : "سحابة مؤسسية مشفرة"}
                  </span>
                </div>

                <h1 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight">
                  {greeting()}، <span className="bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">{displayName}</span> 👋
                </h1>

                <p className="text-xs sm:text-sm text-muted-foreground font-medium max-w-2xl leading-relaxed">
                  {locale === "en"
                    ? "Real-time overview of active candidates, open positions, pipeline stages, and interview metrics."
                    : "متابعة فورية ومباشرة لحركة المتقدمين، الوظائف النشطة، مراحل الفرز الذكي، وتنبيهات النظام اللحظية."}
                </p>
              </div>

              {/* Right: Actions, Live Clock & CTAs */}
              <div className="flex items-center gap-3 flex-wrap shrink-0">
                {unreadNotifications > 0 && (
                  <Link to="/notifications">
                    <Button variant="outline" size="sm" className="gap-2 border-destructive/30 text-destructive hover:bg-destructive/10 rounded-2xl h-11 px-4 text-xs font-bold shadow-xs">
                      <span className="relative flex h-2 w-2">
                ...
                <div className="flex items-center gap-2.5 bg-background/80 dark:bg-slate-900/80 backdrop-blur-md border border-border/80 rounded-2xl px-4 py-2.5 shadow-xs font-bold text-xs text-foreground">
                ...
                <Link to="/jobs?action=new">
                  <Button className="rounded-2xl h-11 px-6 text-xs font-black gap-2 bg-gradient-to-r from-emerald-600 via-emerald-600 to-teal-700 text-white hover:from-emerald-500 hover:to-teal-600 shadow-md shadow-emerald-600/25 transition-all hover:scale-[1.02] active:scale-[0.98]">
                    <Zap className="w-4 h-4 text-amber-300" />
                    <span>{locale === "en" ? "+ Post New Job" : "+ إضافة شاغر جديد"}</span>
                  </Button>
                </Link>
              </div>
            </div>
        ...
        <motion.div variants={item}>
          <div className="rounded-3xl border border-border/70 p-4 sm:p-5 bg-card/80 backdrop-blur-xl shadow-xs">
            <div className="flex items-center justify-between gap-2 mb-3.5">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                  <Timer className="w-4 h-4" />
                </div>
                <span className="text-sm font-black text-foreground">{locale === "en" ? "Today's Operations Pulse" : "نبض عمليات اليوم اللحظي"}</span>
              </div>
              <div className="flex items-center gap-1.5">
                ...
                <Badge variant="outline" className="text-[10px] font-bold rounded-full border-border/70 bg-muted/40">
                  {new Date().toLocaleDateString(locale === "en" ? "en-US" : "ar-SA", { day: "numeric", month: "short", year: "numeric" })}
                </Badge>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              {[
                { label: locale === "en" ? "New Candidates" : "مرشحون جدد", value: productivity.todayCandidates, icon: Users, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500/5 hover:bg-emerald-500/10 border-emerald-500/20", type: "conversion" as const },
                { label: locale === "en" ? "Interviews Today" : "مقابلات اليوم", value: productivity.todayInterviews, icon: Video, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-500/5 hover:bg-amber-500/10 border-amber-500/20", type: "timeToHire" as const },
                { label: locale === "en" ? "This Week" : "هذا الأسبوع", value: productivity.weekCandidates, icon: TrendingUp, color: "text-violet-600 dark:text-violet-400", bg: "bg-violet-500/5 hover:bg-violet-500/10 border-violet-500/20", type: "conversion" as const },
                { label: locale === "en" ? "Week Interviews" : "مقابلات الأسبوع", value: productivity.weekInterviews, icon: Calendar, color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-500/5 hover:bg-blue-500/10 border-blue-500/20", type: "timeToHire" as const },
                { label: locale === "en" ? "Avg Time to Hire" : "متوسط سرعة التوظيف", value: productivity.avgTimeToHire, icon: Clock, color: "text-teal-600 dark:text-teal-400", bg: "bg-teal-500/5 hover:bg-teal-500/10 border-teal-500/20", suffix: locale === "en" ? " days" : " يوم", type: "timeToHire" as const },
              ].map((m, i) => (
                <motion.div
                  key={i}
                  whileHover={{ scale: 1.025, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => setSelectedKpi(m.type)}
                  className={cn("rounded-2xl p-3.5 text-center cursor-pointer border transition-all duration-200 shadow-2xs group", m.bg)}
                >
                  <m.icon className={cn("w-5 h-5 mx-auto mb-1.5 group-hover:scale-110 transition-transform", m.color)} />
                  <p className={cn("text-2xl font-black tabular-nums tracking-tight", m.color)}>{m.value}{m.suffix || ""}</p>
                  <p className="text-[11px] text-muted-foreground font-bold mt-1">{m.label}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        {/* 4 Primary Bento KPI Stat Cards */}
          ...
          {[
            {
              ...
              subtitle: `${t("common.from")} ${allJobs.length} ${locale === "en" ? "total positions" : "إجمالي الشواغر"}`,
              iconColor: "text-emerald-600 dark:text-emerald-400",
              iconBg: "bg-emerald-500/15 border-emerald-500/30",
              cardBg: "bg-gradient-to-br from-emerald-500/[0.08] via-emerald-500/[0.03] to-transparent border-emerald-500/25 hover:border-emerald-500/50 shadow-emerald-500/5",
              progress: allJobs.length > 0 ? Math.round(((stats?.activeJobs ?? 0) / allJobs.length) * 100) : 0,
              progressLabel: locale === "en" ? "Active Ratio" : "نسبة الشواغر النشطة",
              type: "fillRate" as const,
            },
            {
              icon: Users,
              title: t("dashboard.totalCandidates"),
              value: stats?.totalCandidates ?? 0,
              subtitle: weeklyTrend.change >= 0 ? `+${weeklyTrend.change}% ${t("dashboard.thisWeek")}` : `${weeklyTrend.change}% ${t("dashboard.thisWeek")}`,
              trend: weeklyTrend.change,
              iconColor: "text-violet-600 dark:text-violet-400",
              iconBg: "bg-violet-500/15 border-violet-500/30",
              cardBg: "bg-gradient-to-br from-violet-500/[0.08] via-violet-500/[0.03] to-transparent border-violet-500/25 hover:border-violet-500/50 shadow-violet-500/5",
              progress: Math.min(100, Math.round(((stats?.totalCandidates ?? 0) / 100) * 100)),
              progressLabel: locale === "en" ? "Talent Flow" : "حجم تدفق المواهب",
            ...
            },
            {
              ...
              value: stats?.hired ?? 0,
              subtitle: `${kpis.conversionRate}% ${t("dashboard.conversionRate")}`,
              iconColor: "text-teal-600 dark:text-teal-400",
              iconBg: "bg-teal-500/15 border-teal-500/30",
              cardBg: "bg-gradient-to-br from-teal-500/[0.08] via-teal-500/[0.03] to-transparent border-teal-500/25 hover:border-teal-500/50 shadow-teal-500/5",
              progress: kpis.conversionRate,
              progressLabel: locale === "en" ? "Conversion Rate" : "معدل الإنجاز النهائي",
              type: "offers" as const,
            },
            {
              icon: Calendar,
              title: t("dashboard.scheduledInterviews"),
              value: allInterviews.filter(i => i.status === "مجدولة").length,
              subtitle: `${allInterviews.filter(i => i.status === "مكتملة").length} ${t("dashboard.completed")}`,
              iconColor: "text-amber-600 dark:text-amber-400",
              iconBg: "bg-amber-500/15 border-amber-500/30",
              cardBg: "bg-gradient-to-br from-amber-500/[0.08] via-amber-500/[0.03] to-transparent border-amber-500/25 hover:border-amber-500/50 shadow-amber-500/5",
              progress: allInterviews.length > 0 ? Math.round((allInterviews.filter(i => i.status === "مكتملة").length / allInterviews.length) * 100) : 0,
              progressLabel: locale === "en" ? "Completion Rate" : "نسبة إتمام المقابلات",
              type: "timeToHire" as const,
            },
          ].map((stat, i) => (
            <motion.div
              key={stat.title}
              ...
              onClick={() => setSelectedKpi(stat.type)}
              className="cursor-pointer"
            >
              <div className={cn("rounded-3xl border p-5 sm:p-6 transition-all duration-300 shadow-sm relative overflow-hidden group", stat.cardBg)}>
                <div className="flex items-start justify-between">
                  <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center border shadow-xs group-hover:scale-105 transition-transform", stat.iconBg)}>
                    <stat.icon className={cn("w-6 h-6", stat.iconColor)} />
                  </div>
                  {stat.trend !== undefined ? (
                    <Badge variant="outline" className={cn("text-xs font-black rounded-full px-2.5 py-0.5", stat.trend >= 0 ? "text-emerald-600 border-emerald-500/30 bg-emerald-500/10" : "text-destructive border-destructive/30 bg-destructive/10")}>
                      {stat.trend >= 0 ? <ArrowUp className="w-3.5 h-3.5 ml-0.5" /> : <ArrowDown className="w-3.5 h-3.5 ml-0.5" />}
                      {Math.abs(stat.trend)}%
                    </Badge>
                  ) : (
                    <span className="text-[11px] font-black px-2.5 py-0.5 rounded-full bg-muted/60 text-muted-foreground border border-border/40">
                      KPI
                    </span>
                  )}
                </div>
                ...
                <div className="mt-4 space-y-1">
                  ...
                  <p className="text-sm font-black text-foreground">{stat.title}</p>
                ...
                <div className="mt-4 pt-3 border-t border-border/40 space-y-1.5">
                  <div className="flex items-center justify-between text-[10px] font-bold text-muted-foreground">
                    ...
                    <span className={cn("font-black", stat.iconColor)}>{stat.progress}%</span>
                  </div>
                  <div className="w-full h-1.5 rounded-full bg-muted/60 overflow-hidden">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min(100, Math.max(8, stat.progress))}%` }}
                      transition={{ duration: 0.8, delay: 0.2 + i * 0.1 }}
                      className={cn("h-full rounded-full", stat.iconColor.includes("emerald") ? "bg-emerald-500" : stat.iconColor.includes("violet") ? "bg-violet-500" : stat.iconColor.includes("teal") ? "bg-teal-500" : "bg-amber-500")}
                    />
                  </div>
                </div>
              </div>
        ...
        <motion.div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
            {[
              { label: t("dashboard.postJob"), icon: Briefcase, color: "text-emerald-600 dark:text-emerald-400 border-emerald-500/25 hover:bg-emerald-500/10 hover:border-emerald-500/50", path: "/jobs?action=new" },
              { label: t("dashboard.candidates"), icon: Users, color: "text-violet-600 dark:text-violet-400 border-violet-500/25 hover:bg-violet-500/10 hover:border-violet-500/50", path: "/candidates" },
              { label: t("dashboard.pipeline"), icon: Kanban, color: "text-blue-600 dark:text-blue-400 border-blue-500/25 hover:bg-blue-500/10 hover:border-blue-500/50", path: "/pipeline" },
              { label: t("dashboard.scheduleInterview"), icon: Calendar, color: "text-amber-600 dark:text-amber-400 border-amber-500/25 hover:bg-amber-500/10 hover:border-amber-500/50", path: "/interviews" },
              { label: t("dashboard.offers"), icon: FileText, color: "text-teal-600 dark:text-teal-400 border-teal-500/25 hover:bg-teal-500/10 hover:border-teal-500/50", path: "/offers" },
              { label: t("dashboard.reports"), icon: BarChart3, color: "text-foreground border-border/70 hover:bg-muted/50 hover:border-emerald-500/40", path: "/reports" },
            ].map((action) => (
              <Link key={action.label} to={action.path}>
                <motion.div
                  whileHover={{ scale: 1.03, y: -2 }}
                  whileTap={{ scale: 0.97 }}
                  className={cn("group flex flex-col items-center gap-2 rounded-2xl p-4 font-bold text-xs transition-all cursor-pointer text-center bg-card/70 backdrop-blur-md border shadow-2xs", action.color)}
                >
                  <div className="p-2.5 rounded-xl bg-muted/60 border border-border/60 shadow-2xs group-hover:scale-110 transition-transform">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div variants={item} whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
            <Card className="relative rounded-3xl border border-border/70 bg-card/75 backdrop-blur-xl shadow-sm h-full overflow-hidden flex flex-col justify-between">
              <CardHeader className="pb-3 border-b border-border/40">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-black flex items-center gap-2 text-foreground">
                    ...
                    <span>{t("dashboard.pipelineStages")}</span>
                  </CardTitle>
                  <Link to="/pipeline" className="text-xs text-primary hover:underline flex items-center gap-1 font-bold">
              ...
              <CardContent className="pt-4 flex-1">
                {allCandidates.length === 0 ? (
                  <div className="h-48 flex items-center justify-center text-muted-foreground text-sm font-semibold">{t("common.noData")}</div>
                ) : (
                  <div className="space-y-3.5">
                    {stageData.map((s, i) => {
                      const maxCount = Math.max(...stageData.map(sd => sd.count), 1);
                      const width = Math.max((s.count / maxCount) * 100, 5);
                      const percentOfTotal = allCandidates.length > 0 ? Math.round((s.count / allCandidates.length) * 100) : 0;
                      return (
                        <div key={s.stage} className="space-y-1.5 group">
                          <div className="flex items-center justify-between text-xs font-bold">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-[10px] text-muted-foreground font-semibold">({percentOfTotal}%)</span>
                              <span className="font-black text-foreground tabular-nums px-2 py-0.5 rounded-md bg-muted/60 text-xs">
                                {s.count}
                              </span>
                            </div>
                          </div>
                          <div className="h-3 bg-muted/50 rounded-full overflow-hidden border border-border/30 p-0.5">
                            <motion.div
                              initial={{ width: 0 }}
                              animate={{ width: `${width}%` }}
            ...
            <Card className="relative rounded-3xl border border-border/70 bg-card/75 backdrop-blur-xl shadow-sm h-full overflow-hidden flex flex-col justify-between">
              <CardHeader className="pb-3 border-b border-border/40">
                <CardTitle className="text-base font-black flex items-center gap-2 text-foreground">
                  ...
                  <span>{t("dashboard.candidateSources")}</span>
                </CardTitle>
              </CardHeader>
                ...
                {sourceData.length === 0 ? (
                  ...
                  <>
                    <ResponsiveContainer width="100%" height={150}>
                      <PieChart>
                        <Pie data={sourceData} cx="50%" cy="50%" innerRadius={42} outerRadius={65} dataKey="value" strokeWidth={0} paddingAngle={4}>
                    ...
                    <div className="space-y-2 mt-4 pt-3 border-t border-border/40">
                      {sourceData.slice(0, 4).map((s, i) => (
                        <div key={i} className="flex items-center justify-between text-xs font-bold p-1.5 rounded-xl hover:bg-muted/40 transition-colors">
                          <div className="flex items-center gap-2">
            ...
            <Card className="relative rounded-3xl border border-border/70 bg-card/75 backdrop-blur-xl shadow-sm h-full overflow-hidden flex flex-col justify-between">
              <CardHeader className="pb-3 border-b border-border/40">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-black flex items-center gap-2 text-foreground">
                    ...
                    <span>{t("dashboard.offersSummary")}</span>
                  </CardTitle>
                  <Link to="/offers" className="text-xs text-primary hover:underline flex items-center gap-1 font-bold">
                    <span>{t("common.viewAll")}</span>
                    <ChevronRight className="w-3.5 h-3.5" />
                  </Link>
                </div>
              ...
              <CardContent className="pt-4 flex-1 flex flex-col justify-between">
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: t("dashboard.totalOffers"), value: offersStats.total, icon: FileText, color: "text-foreground", bg: "bg-muted/40 border border-border/50" },
                    { label: t("dashboard.sentOffers"), value: offersStats.sent, icon: ArrowUpRight, color: "text-primary", bg: "bg-primary/5 border border-primary/15" },
                    { label: t("dashboard.acceptedOffers"), value: offersStats.accepted, icon: CheckCircle2, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500/5 border border-emerald-500/15" },
                    { label: t("dashboard.rejectedOffers"), value: offersStats.rejected, icon: AlertCircle, color: "text-destructive", bg: "bg-destructive/5 border border-destructive/15" },
                  ].map((s, i) => (
                    <motion.div
                      key={i}
                      whileHover={{ scale: 1.03, y: -2 }}
                      whileTap={{ scale: 0.97 }}
                      className={cn("rounded-2xl p-3.5 text-center transition-all cursor-default shadow-2xs group", s.bg)}
                    >
                      <s.icon className={cn("w-4 h-4 mx-auto mb-1.5 group-hover:scale-110 transition-transform", s.color)} />
                ...
                <div className="mt-4 pt-3 border-t border-border/40 space-y-1.5">
                  <div className="flex justify-between text-xs font-bold">
                    ...
                    <span className="font-black text-emerald-600 dark:text-emerald-400">{offersStats.acceptanceRate}%</span>
                  </div>
                  <div className="w-full h-2 bg-muted/60 rounded-full overflow-hidden border border-border/30">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${offersStats.acceptanceRate}%` }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div variants={item} whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
            <Card className="relative rounded-3xl border border-border/70 bg-card/75 backdrop-blur-xl shadow-sm h-full overflow-hidden flex flex-col justify-between">
              <CardHeader className="pb-3 border-b border-border/40 bg-amber-500/[0.03]">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-black flex items-center gap-2 text-foreground">
                    <div className="w-8 h-8 rounded-xl bg-amber-500/15 text-amber-600 dark:text-amber-400 flex items-center justify-center">
                      <Calendar className="w-4 h-4" />
                    </div>
                    <span>{t("dashboard.upcomingInterviews")}</span>
                  </CardTitle>
                  <Link to="/interviews" className="text-xs text-primary hover:underline flex items-center gap-1 font-bold">
                    <span>{t("common.viewAll")}</span>
                    <ChevronRight className="w-3.5 h-3.5" />
                  </Link>
                </div>
              ...
              <CardContent className="pt-3 flex-1">
                {todayInterviews.length === 0 ? (
                  <div className="text-center py-10 space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground font-bold">{t("dashboard.noScheduledInterviews")}</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {todayInterviews.map((interview) => (
                      <div
                        key={interview.id}
                        className="p-3 rounded-2xl border border-border/60 bg-muted/20 hover:bg-muted/40 hover:border-amber-500/30 transition-all flex items-center justify-between gap-3 shadow-2xs group"
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="font-bold text-xs text-foreground truncate">{interview.candidate_name}</p>
                            <p className="text-[11px] text-muted-foreground truncate">{interview.position}</p>
                          </div>
                        </div>

                        <div className="text-left shrink-0 space-y-1">
                          <span className="text-[11px] font-black text-amber-600 dark:text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-md border border-amber-500/20 block text-center">
                            {interview.time?.slice(0, 5)}
                          </span>
                          <Link
                            to={`/video-room/${interview.id}`}
          ...
          {/* Recent Candidates with AI Match */}
          <motion.div variants={item} whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
            <Card className="relative rounded-3xl border border-border/70 bg-card/75 backdrop-blur-xl shadow-sm h-full overflow-hidden flex flex-col justify-between">
              <CardHeader className="pb-3 border-b border-border/40 bg-emerald-500/[0.03]">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-black flex items-center gap-2 text-foreground">
                    <div className="w-8 h-8 rounded-xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                      <Users className="w-4 h-4" />
                    </div>
                    <span>{t("dashboard.recentCandidates")}</span>
                  </CardTitle>
                  <Link to="/candidates" className="text-xs text-primary hover:underline flex items-center gap-1 font-bold">
                    <span>{t("common.viewAll")}</span>
                    <ChevronRight className="w-3.5 h-3.5" />
                  </Link>
                </div>
              ...
              <CardContent className="pt-3 flex-1">
                {recentCandidates.length === 0 ? (
                  <div className="text-center py-10 space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground font-bold">{t("candidates.noCandidates")}</p>
                  </div>
                ) : (
                  <div className="space-y-2.5">
                    {recentCandidates.map((c) => (
                      <Link
                        key={c.id}
                        to={`/candidates/${c.id}`}
                        className="flex items-center justify-between gap-3 p-2.5 rounded-2xl border border-border/50 bg-muted/20 hover:bg-muted/40 hover:border-emerald-500/30 transition-all group shadow-2xs"
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-black text-[11px] flex items-center justify-center shrink-0 shadow-2xs group-hover:scale-105 transition-transform">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {(c as any).ai_score != null && (
                            <span className="flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded-md text-[10px] font-black">
                              <Star className="w-2.5 h-2.5 fill-emerald-500" />
                              <span>{(c as any).ai_score}%</span>
                            </span>
                          )}
                          <Badge variant="outline" className={cn("text-[10px] font-bold border", statusColors[c.status] || "")}>
                            {c.status}
                          </Badge>
                        </div>
                      </Link>
          ...
          <motion.div variants={item} whileHover={{ y: -4 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
            <Card className="relative rounded-3xl border border-border/70 bg-card/75 backdrop-blur-xl shadow-sm h-full overflow-hidden flex flex-col justify-between">
              <CardHeader className="pb-3 border-b border-border/40 bg-blue-500/[0.03]">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base font-black flex items-center gap-2 text-foreground">
                    <div className="w-8 h-8 rounded-xl bg-blue-500/15 text-blue-600 dark:text-blue-400 flex items-center justify-center">
                  ...
                  <Link to="/notifications" className="text-xs text-primary hover:underline flex items-center gap-1 font-bold">
                    <span>{t("common.viewAll")}</span>
                    <ChevronRight className="w-3.5 h-3.5" />
                  </Link>
                </div>
              ...
              <CardContent className="pt-3 flex-1">
                {allNotifications.length === 0 ? (
                  <div className="text-center py-10 space-y-2">
                    <Activity className="w-10 h-10 mx-auto text-muted-foreground/30 animate-pulse" />
                    <p className="text-xs text-muted-foreground font-bold">{t("dashboard.noActivity")}</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {allNotifications.slice(0, 5).map((n) => {
                      const typeColors: Record<string, string> = {
                        ...
                        stage_change: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20",
                        rejection: "bg-destructive/10 text-destructive border-destructive/20",
                        offer: "bg-teal-500/15 text-teal-600 border-teal-500/30",
                        interview: "bg-amber-500/10 text-amber-600 border-amber-500/20",
                        system: "bg-muted text-muted-foreground border-border/20",
                      };
                      ...
                      return (
                        <div
                          key={n.id}
                          className="flex items-start gap-2.5 p-2.5 rounded-2xl border border-border/40 bg-muted/20 hover:bg-muted/40 transition-colors shadow-2xs"
                        >
                          <div className={cn("w-7 h-7 rounded-lg flex items-center justify-center shrink-0 border mt-0.5", typeColors[n.type] || "bg-muted")}>
                            <Icon className="w-3.5 h-3.5" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={cn("text-xs leading-relaxed truncate", !n.read ? "font-black text-foreground" : "text-foreground/80 font-medium")}>
                              {n.title}
                            </p>
                            <p className="text-[10px] text-muted-foreground font-semibold mt-0.5">{formatTime(n.created_at)}</p>
                          </div>
                          {!n.read && <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0 mt-1 shadow-xs animate-pulse" />}
                        </div>
                      );
                    })}
                  </div>
