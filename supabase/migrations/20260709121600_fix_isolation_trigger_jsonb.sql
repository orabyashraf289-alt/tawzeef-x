-- Safely update set_row_company_id trigger function using jsonb conversion to bypass plpgsql record field compile checks
...
BEGIN
  ...
  IF _company_id IS NULL AND TG_TABLE_NAME IN ('applications', 'candidates') THEN
    -- Safely extract job_id using JSONB to bypass plpgsql compile-time type validation of the NEW record
    BEGIN
